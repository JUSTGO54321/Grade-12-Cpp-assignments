...
bool parseNumEmps(char *numEmpsString){

	int i = 0;

 	while (numEmpsString[i]){

		if (numEmpsString[i] <= '9' && numEmpsString[i] >= '0'){
			i++;
		}
		else {
			return false;
...
int getNumEmps (int numEmps, int maxNumBytes){

	char numEmpsString[maxNumBytes];
	bool invalid;

	do {
		invalid = false;
		printf ("Enter the number of employees (max 10000): ");
		fgets (numEmpsString, maxNumBytes, stdin);
		numEmpsString[strlen(numEmpsString) - 1] = 0;

		if (parseNumEmps (numEmpsString) == false){
...
void getEmps(empType *emps, int n){
	char empString[256];
	char filename[] = "empInput.txt";
	FILE *empFile;
	empFile = fopen("empInput.txt", "r");
	...
	if (empFile == NULL){
		...
		int j = 0;
		while (i < n){
			fgets(empString, 256, empFile);
			empString[strlen(empString) - 1] = 0;
			if (parseEmp(empString, &emps[j])){
				i++; 
				j++;
			} else i++;
		}
...
int main(){
	empType emps[10000];
	const int maxNumBytes = 256;
	char parseNumEmps[maxNumBytes];
	int numEmps;

	numEmps = getNumEmps (numEmps, maxNumBytes);
